...
// draws its own full-page shell instead of a `Modal` — an Experience's own setup
// dialog, `LegacyExperienceSetupDialog` — listens for Escape on `window`, which
// means one press would close the dialog AND tear down the screen underneath
// it. The malformed-JSON repair dialog is the common case: `GameSurface` mounts
// it next to the setup wizard precisely so a failed opening stays repairable,
// and dismissing it must not also dismiss the setup the player is recovering.
//
